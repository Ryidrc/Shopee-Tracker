...
export const VideoTrackerView: React.FC<VideoTrackerViewProps> = ({ videoLogs, pricingItems, onAddLog, onEditLog, onDeleteLog }) => {
  ...
  const performanceData = useMemo(() => {
    return videoLogs.map(log => {
      const product = pricingItems.find(p => p.sku === log.sku && p.shopId === log.shopId) 
                   || pricingItems.find(p => p.sku === log.sku); 
      let rating = 'Normal';
  ...
  const thClass = "px-4 py-3 text-left text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider bg-white dark:bg-slate-800 sticky top-0";
  const tdClass = "px-4 py-3 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300 border-b border-slate-50 dark:border-slate-800/50";

  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors">
        <div>
        ...
        <div className="flex gap-4 items-center">
           <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-full">
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${activeTab === 'coverage' ? 'bg-white dark:bg-slate-600 shadow-sm text-shopee-orange' : 'text-slate-500 dark:text-slate-400'}`}
              >
                Coverage
              </button>
              <button 
                onClick={() => setActiveTab('performance')}
                className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${activeTab === 'performance' ? 'bg-white dark:bg-slate-600 shadow-sm text-shopee-orange' : 'text-slate-500 dark:text-slate-400'}`}
              >
                Log
              </button>
           </div>
           ...
           <button 
              onClick={onAddLog}
              className="bg-shopee-orange text-white px-4 py-2 rounded-full text-sm font-bold shadow-sm hover:bg-red-600 transition flex items-center gap-2"
           >
      ...
      {activeTab === 'coverage' ? (
        <div className="space-y-4 animate-fade-in">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-red-50 dark:bg-red-900/20 p-5 rounded-2xl">
                  <div className="text-red-400 text-xs font-bold uppercase tracking-wider">Untouched</div>
              ...
              <div className="bg-green-50 dark:bg-green-900/20 p-5 rounded-2xl">
                  <div className="text-green-500 text-xs font-bold uppercase tracking-wider">Total Uploads</div>
                  <div className="text-2xl font-bold text-slate-800 dark:text-white mt-1">
                      {videoLogs.length}
                  </div>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-2xl">
                   <div className="text-blue-500 text-xs font-bold uppercase tracking-wider">Most Covered</div>
           ...
           <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col transition-colors">
              <div className="p-4 border-b border-slate-50 dark:border-slate-800">
                  <input 
                    ...
                    className="w-full md:w-64 border-none bg-slate-50 dark:bg-slate-700 rounded-full px-4 py-2 text-sm text-slate-900 dark:text-white focus:ring-0"
                  />
              </div>
                ...
                <table className="min-w-full">
                    <thead>
                        <tr>
                            ...
                            <th className={thClass + " text-center"}>Videos</th>
                    ...
                    <tbody className="bg-white dark:bg-slate-800">
                        {inventoryCoverage.map(item => (
                            <tr key={item.sku} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                <td className={tdClass}>
                                    <div className="font-bold text-slate-700 dark:text-white">{item.name}</div>
                                </td>
                                <td className={tdClass + " font-mono text-xs text-slate-400"}>{item.sku}</td>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden animate-fade-in transition-colors">
             <div className="overflow-x-auto">
                <table className="min-w-full">
                    <thead>
                        <tr>
                        ...
                        {performanceData.map(log => {
                            ...
                            return (
                                <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                                    <td className={tdClass + " text-slate-400 text-xs"}>{log.date}</td>
                                    <td className={tdClass}>
                                        <span className="text-[10px] px-2 py-0.5 rounded font-bold text-white whitespace-nowrap" style={{ backgroundColor: shop?.color }}>
                                            {shop?.name}
                                        </span>
                                    </td>
                                    <td className={tdClass}>
                                        <div className="max-w-[150px] truncate font-medium text-slate-800 dark:text-white" title={log.productName}>{log.productName}</div>
                                    </td>
                                    <td className={tdClass}>
                                        <span className="bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-[10px] font-bold uppercase text-slate-500 dark:text-slate-300">
                                            {log.concept}
                                        </span>
                                    </td>
                                    ...
                                    <td className={tdClass}>
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase ${log.ratingColor}`}>
                                            {log.rating}
                                        </span>
                                    </td>
                                    <td className={tdClass + " text-right"}>
                                         <button onClick={() => onEditLog(log)} className="text-slate-300 hover:text-blue-500 transition-colors mr-2 opacity-0 group-hover:opacity-100">
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                         </button>
                                         <button onClick={() => onDeleteLog(log.id)} className="text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
